        ...
        str += `
            <div class="plan-card">
                <div class="plan-left">
                    ...
                    ${loginEmployee ? `` : `
                    ${LOGIN_ADMIN ? `
                        <button class="plan-v2-update-btn" onclick="update('${vo.no}');">수정하기</button>
                        <button class="plan-v2-update-btn" onclick="del('${vo.no}');">삭제하기</button>
                    ` : `
                        <button class="compare-btn" onclick="location.href='/plan/detail/${vo.no}'">가입하러가기</button>
                        <button class="change-btn" onclick="location.href='/main-contract/list'">약정선택하기</button>
                    `}`}
                </div>
                </div>
            </div>
...
function update(no){
    localStorage.setItem("uNo",no);
    location.href = `/plan/admin/update`
}

async function del(no){
    try {
        const resp = await fetch(`/api/plan/admin/delete`,{
            method : "DELETE",
            headers : {
